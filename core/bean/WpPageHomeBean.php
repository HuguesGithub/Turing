...
class WpPageHomeBean extends WpPageBean
{
    ...
    public function getContentPage()
    {
		...
		$strCriteria = '';
		$btnDropDown = '<button type="button" class="btn btn-success btn-outline-dark dropdown-toggle text-white fw-bold" data-trigger="dropdown" aria-expanded="false">&nbsp;</button>';
		while (!empty($arrAllowedRanks)) {
			$rk = array_shift($arrAllowedRanks);
			$ul = $this->getBalise(self::TAG_UL, $strOptions, array('class'=>'dropdown-menu', 'data-criteria-rank'=>$rk));
			$div = $this->getBalise(self::TAG_DIV, $btnDropDown.$ul, array('class'=>'btn-group', 'role'=>'group'));
			$label = $this->getBalise(self::TAG_LABEL, $rk, array('class'=>'btn btn-light btn-outline-dark text-success mb-0'));
